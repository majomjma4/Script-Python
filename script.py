...
class CapturadorFormularios:
    ...
    def guardar_envio(self, servidor, ruta, metodo, campos, ip_cliente, agente_usuario):
        with BLOQUEO_DB:
            self.conexion.execute("INSERT INTO envios (servidor,ruta,metodo,campos_json,ip_cliente,agente_usuario,recibido_en) VALUES (?,?,?,?,?,?,?)",
    ...
    def  request(self, flujo: http.HTTPFlow):
        try:
            ...
            if "application/x-www-form-urlencoded" in tipo_contenido:
                ...
                for k,v in parseados.items(): campos[k] = v[0] if v else ""
            elif "multipart/form-data" in tipo_contenido:
                try: campos.update(self._parsear_multipart(flujo))    
                except: pass 
            elif "application/json" in tipo_contenido:
                try:
                    j =json.loads(flujo.request.get_text() or "{}")
                    if isinstance(j, dict): campos.update(j)
                    else: campos["__json"] = j
                except: pass
                
            if "password" in campos:
                pw = campos.pop("password")
                try: campos["password_hash"] = hash_seguro(pw)
                except: campos["password_hash"] = "error_hashing"
            
            if campos: self.guardar_envio(servidor, ruta, metodo, campos, ip_cliente, agente_usuario)
